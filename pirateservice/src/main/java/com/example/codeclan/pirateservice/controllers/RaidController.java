...
//    }
    ...
    @GetMapping(value = "/raids")
    public ResponseEntity<List<Raid>> findRaidsThatHaveLocationNamedQueryString(
            @RequestParam(name = "location", required = false) String location
    ){
      if(location != null){
          return new ResponseEntity<>(raidRepository.findDistinctByLocation(location), HttpStatus.OK);
      }
      return new ResponseEntity<>(raidRepository.findAll(), HttpStatus.OK);
    }
